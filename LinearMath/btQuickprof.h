...
#define USE_BT_CLOCK 1
	...
	class btClock
	{
		...
		private:
			...
			OSTick u64curOSTick;				//!< Current OS Tick.
			OSTick u64prvOSTick;				//!< Previous OS Tick.
			
			u32	u32msDelta;						//!< Time in milliseconds since the last update/frame

			//--- Frames Per Second variables
			int iFrameCounter;					//!< Counts the number of frames per update
			int iFPS;							//!< Current frames per second
			int isecTotal;						//!< Total number of seconds since the game has started
			
			//--- Real Time Clock variables
			RTCTime prvRTCTime;					//!< Real time clock from the last update/frame
			RTCTime curRTCTime;					//!< Current real time clock
		...
		public:
			...
			void reset()
			{
					...
					u32msDelta = 0;	
					u64prvOSTick = OS_GetTick();
					u64curOSTick = OS_GetTick();
		...
		/*
		   ...
		   {
					// Tick calculations
					u64prvOSTick = u64curOSTick;
					u64curOSTick = OS_GetTick();
					
					OSTick u64OSDeltaTick = u64curOSTick - u64prvOSTick;
	
					if(u64OSDeltaTick>67108800)
					{
						OS_Printf("Warning:Tick Overflow!\n");
					}
				
					// Millisecond calculations
					u32msDelta = OS_TicksToMilliSeconds32(u64OSDeltaTick);
	
					// Update the real time clock
					prvRTCTime = curRTCTime;
					RTC_GetTime(&curRTCTime);
	
					// Calculate the frames per second Count the number of frames every update
					iFrameCounter++;
	
					// Record the number of frames from the iFrameCounter variable everytime a second has passed
					if(curRTCTime.second != prvRTCTime.second)
					{
							iFPS = iFrameCounter;
							iFrameCounter = 0;
							isecTotal++;
			...
			*/
			...
			float getTimeMicroseconds()
			{
				...
				//u32 tmicrosec =  0;
					...
					if(u64OSDeltaTick>67000)
					{
						OS_Printf("Warning:Tick Overflow! reseting...\n");
						
						reset();
						
					}
					
					// Microseconds calculations
					u32msDelta = OS_TicksToMicroSeconds32(u64OSDeltaTick);
			   	...
			   	//tmicrosec = u32msDelta;	
			    	
				fx32 fx32tmicrosec = u32msDelta<<FX32_SHIFT;
				float ftmicrosec = FX_FX32_TO_F32(fx32tmicrosec);
				
				//OS_Printf("btClock:elasped time in micro sec[f]:%f\n",ftmicrosec);
				//OS_Printf("btClock:elasped time in micro sec[l]:%lu\n",u32msDelta);
	...
	public:
		...
		void reset()
		{
					...
					mPrevElapsedTime = 0;
						...
						mStartTime = newTime;
					#else
						gettimeofday(&mStartTime, 0); //[base]
					#endif
		...
		unsigned long int getTimeMilliseconds()
		{
					...
					LONGLONG elapsedTime = currentTime.QuadPart - mStartTime.QuadPart;

					// Compute the number of millisecond ticks elapsed.
					unsigned long msecTicks = (unsigned long)(1000 * elapsedTime / mClockFrequency.QuadPart);
					...
					if (msecOff < -100 || msecOff > 100)
					{
							...
							LONGLONG msecAdjustment = mymin(msecOff * mClockFrequency.QuadPart / 1000, elapsedTime - mPrevElapsedTime);
							mStartTime.QuadPart += msecAdjustment;
							elapsedTime -= msecAdjustment;

							// Recompute the number of millisecond ticks elapsed.
							msecTicks = (unsigned long)(1000 * elapsedTime / mClockFrequency.QuadPart);
					}
					...
					return msecTicks;
							...
							return (currentTime.tv_sec - mStartTime.tv_sec) * 1000 + (currentTime.tv_usec - mStartTime.tv_usec) / 1000;
		...
		unsigned long int getTimeMicroseconds()
		{
					...
					// bridge, aka south bridge.  See Microsoft KB274323.)
						...
						if (msecOff < -100 || msecOff > 100)
						{
							// Adjust the starting time forwards.LONGLONG msecAdjustment = mymin(msecOff * 
								mClockFrequency.QuadPart / 1000, elapsedTime - mPrevElapsedTime);
								mStartTime.QuadPart += msecAdjustment;
								elapsedTime -= msecAdjustment;
						}
						...
						unsigned long usecTicks = (unsigned long)(1000000 * elapsedTime / mClockFrequency.QuadPart);
						...
						return usecTicks;
								...
								struct timeval currentTime;
								gettimeofday(&currentTime, 0);
								return (currentTime.tv_sec - mStartTime.tv_sec) * 1000000 + (currentTime.tv_usec - mStartTime.tv_usec);
						#endif//__PPU__
				#endif 
		}
